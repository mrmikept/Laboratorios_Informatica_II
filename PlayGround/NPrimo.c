...
int main()
{
    ...
    {
        if (v1 > v2)
        { // Troca de v1 por v2 se v1 > v2 (Ordenação.)
            v1 = v1 + v2;
            v2 = v1 - v2;
            v1 = v1 - v2;
        }
...
int verificPrim(int n)
{                     // NUmeros primos são aqueles que somente são divisiveis por 1 e por ele mesmo. Então vamos usar um ciclo de for para verificar se existe
