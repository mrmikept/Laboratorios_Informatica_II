...
int main()
{
    ...
    { // Primeiro temos de ordenar os tres valores de input para depois podermos usar a funcao verFactor(Verificar se é Factorial)
        if ((a > b) & (a > c))
        {
            maior = a;
            if (b > c)
...
int verFactor(int n)
{ // Vamos usar um ciclo for para verificar se a variavel n e um numero fatorial.
    int x = 1;
    for (int i = 1; x <= n; i++)
    {
        x = x * i;
